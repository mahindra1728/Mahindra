...
%         
        ...
        for k=1:length(Folder1)
            ...
            disp(path)
            counts_noise_new = counts_noise{destination,1}{k};
            filt_data = load(strcat(path,'\eeg_data_Adaptive_filter_WI_2degree.txt'));
            raw_data  = load(strcat(path,'\eeg_data.txt'));
            if (destination == 1 && k==5) || (destination == 2 && k==6)|| (destination == 5 && k==8) || (destination == 3 && k==8) || (destination == 6 && (k==2 || k==8)) ||(destination == 8 && (k==3 || k==5 || k==1)) || (destination == 10 && (k==5 || k==6)) || (destination == 9 && (k==2 || k==8))
