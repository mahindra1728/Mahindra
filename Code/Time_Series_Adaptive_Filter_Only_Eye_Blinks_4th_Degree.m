...
for destination = 1:10
    ...
    for k=1:length(Folder)
        ...
        for j = 1:length(counts_noise_new(:,1))
            row = j;
            if abs(counts_noise_new(row,1) - counts_noise_new(row,2)-1) >= 15
                row = j;
...
%         end
        ...
        fnm =[Parent_Folder strcat(participant,'.',num2str(k)) '\' 'eeg_data_Adaptive_filter_WI_4degree_Only_Eye_Blinks.txt'];
        fid = fopen(fnm,'wt');
        fprintf(fid,'%.2f\n',eeg_Adaptive_filtered_data);
        fclose(fid);
        save('eeg_data_Adaptive_filter_WI_4degree_Only_Eye_Blinks.txt','data_to_be_filtered','-ascii')
    end
end