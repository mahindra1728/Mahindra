...
for destination = 1:10
    ...
    warning('off')
    format short g
    Main_Folder = 'D:\Mahindra\4_points_to_10seconds_expt_file\4_points_new\4_point\EEGDATA\';
...
% %                 print(figure(L),fullfile(path,['PSD_expt_',num2str(k),'_Task_',num2str(m),'_part_all', '.png']),'-dpng','-r800');             
                ...
                filename = strcat(path,'Experiment','.xlsx');
...
%                         subplot(3,2,[5,6])
%                         x = [(start_scenario(1,1)+ start_alarm(1,1))/2,(start_alarm(1,1) + start_scenario(4,1))/2 ,(start_scenario(4,1)+start_alarm(length(start_alarm),1))/2,(start_alarm(length(start_alarm),1)+start_scenario(length(start_scenario),1))/2];                         y = PSD;
