    ...
    for i = 1
        ...
        for k=1:length(Folder)
            ...
            for m=1:6
                ...
                for q = 1:7
                    read={strcat(path,'PSD_After_AF_LMS'),strcat('Sheet',num2str(q))};    %% EXPERIMENT+REST SHEET CONTAINING THETA, ALPHA AND BETA
                ...
                if sum(strcmpi(alarm_name1{k,m}(1,1), 'F102_low'))>=1 % F102_LOW
                    ...
                    PSD_theta{k,m} = PSD_raw{k,m}(1,3:length(PSD_raw{k,m}(1,:)));
%                     PSD_beta{k,m} = mean(PSD_raw{k,m}(1,3:length(PSD_raw{k,m}(1,:))));
%                     PSD_alpha{k,m} = mean(PSD_raw{k,m}(1,3:length(PSD_raw{k,m}(1,:))));
...
%                 PSD_BETA_Rest{destination,1}{k} = mean(PSD_raw_rest{destination,1}{k}(14:31,:));
                PSD_THETA_Rest{destination,1}{k} =PSD_raw_rest{destination,1}{k};
...
%                 PSD_BETA_Observation{destination,1}{k,m}  = mean(PSD_raw{k,m}(14:31,1)); % PSD of Observation period only
                PSD_THETA_Observation{destination,1}{k,m} = PSD_raw{k,m};
...
%                 PSD_BETA_decision{destination,1}{k,m} =  mean(PSD_raw{k,m}(14:31,2)); % PSD of decison only
                PSD_THETA_decision{destination,1}{k,m} = PSD_raw{k,m};
