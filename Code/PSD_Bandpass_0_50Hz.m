...
%             filteredData_data = filtfilt(b, a, ip_sig); % Apply filter to data using zero-phase filtering
            ...
            fnm =[Parent_Folder strcat(participant,'.',num2str(k)) '\' 'eeg_data_BP_0_50.txt'];
            fid = fopen(fnm,'wt');
            fprintf(fid,'%.2f\n',filteredData_data);
            fclose(fid);
            save('eeg_data_BP_0_50.txt','filteredData_data','-ascii')
        end
        ...
        for k=1:length(Folder)
            ...
            for m =1:6
                ...
                EEG = pop_importdata('dataformat','ascii','nbchan',1,'data',strcat(Parent_Folder,participant,'.',num2str(k),'\eeg_data_BP_0_50.txt'),'setname','applicant','srate',512,'pnts',1,'xmin',0);
                [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 0,'gui','off');
                EEG = eeg_checkset( EEG );
                start_scenario{k,1} = eval(strcat(participant,sch,'_expt_',num2str(k),'_mouse_click',num2str(m)));
                ...
                N = ceil(diff_rest/10);
                time2 = 10;
                clear x_1 y_1 time_rest S_theta_rest S_alpha_rest S_beta_rest mean_time_rest time_mean_rest time_mean_plotting 
                clear x y time S_theta S_alpha S_beta mean_time time_action_mean time_mean time_action time time_rest
                for i = 1:N-1
                    time_rest{i} = ((i-1)*10)+1:((i-1)*10)+time2;
                    pop_spectopo(EEG, 1, [time_rest{1,i}(1,1)*512 time_rest{1,i}(1,length(time_rest{1,i}))*512], 'EEG' , 'freqrange',[2 30],'electrodes','on'); % First slider action to completion of scenario
                    ...
                    time_mean_rest{i} = floor(mean(cell2mat(time_rest(i))));
                end
                ...
                pop_spectopo(EEG, 1, [time_rest{1,N}(1,1)*512 time_rest{1,N}(1,length(time_rest{1,N}))*512], 'EEG' , 'freqrange',[2 30],'electrodes','on'); % First slider action to completion of scenario
                h4 = findobj(gca,'Type','line');
                x_1{N}=get(h4,'Xdata');
                y_1{N}=get(h4,'Ydata');
                ...
                figure(L)
                subplot(2,1,1)
                pop_spectopo(EEG, 1, [(diff_rest + start_scenario{k,1}(1,1))*512  (diff_rest+ start_alarm(1,1))*512], 'EEG' , 'freqrange',[2 30],'electrodes','on'); % Start to Start of task
                ...
                subplot(2,1,2)
                if start_scenario{k,1}(4,1) - start_alarm(1,1) == 1 || start_scenario{k,1}(4,1) - start_alarm(1,1) == 0
                    time2 = start_scenario{k,1}(4,1) + 2;
                else
                    time2 = start_scenario{k,1}(4,1);
                end
                pop_spectopo(EEG, 1, [(diff_rest + start_alarm(1,1))*512 (diff_rest + time2)*512], 'EEG' , 'freqrange',[2 30],'electrodes','on'); % Task to 1st slider action
                ...
                diff = abs(start_scenario{k,1}(4,1) - start_scenario{k,1}(length(start_scenario{k,1}),1));
                N = ceil(diff/10);
                time1 = diff_rest + start_scenario{k,1}(4,1);
                time2 = diff_rest + 10+start_scenario{k,1}(4,1);
                for i = 1:N-1
                    time_action{i} = ((i-1)*10)+time1+1:((i-1)*10)+time2;
                    pop_spectopo(EEG, 1, [time_action{1,i}(1,1)*512 time_action{1,i}(1,length(time_action{1,i}))*512], 'EEG' , 'freqrange',[2 30],'electrodes','on'); % First slider action to completion of scenario
                    h4 = findobj(gca,'Type','line');
                    x{i}=get(h4,'Xdata');
                    y{i}=get(h4,'Ydata');
                    close all
                ...
                time_action{N} = time_action{1,N-1}(1,length(time_action{1,N-1}))+1 : (diff_rest + start_scenario{k,1}(length(start_scenario{k,1}),1));
                if length(time_action{1,N})<3
                    
                    time_action{N} = time_action{1,N-1}(1,length(time_action{1,N-1}))-3 : diff_rest+start_scenario{k,1}(length(start_scenario{k,1}),1);
                else
                    time_action{N} = time_action{1,N-1}(1,length(time_action{1,N-1}))+1 : (diff_rest + start_scenario{k,1}(length(start_scenario{k,1}),1));
                end
                ...
                diff = abs(start_scenario{k,1}(4,1) - start_scenario{k,1}(length(start_scenario{k,1}),1));
                N = ceil(diff/10);
                time1 = start_scenario{k,1}(4,1);
                time2 = 10+start_scenario{k,1}(4,1);
                for i = 1:N-1
                    time{i} = ((i-1)*10)+time1+1:((i-1)*10)+time2;
                    time_mean_plotting{i} = floor(mean(cell2mat(time(i))));
                end
                time{N} = time{1,N-1}(1,length(time{1,N-1}))+1 : (start_scenario{k,1}(length(start_scenario{k,1}),1));
                if length(time{1,N})<3
                    
                    time{N} = time{1,N-1}(1,length(time{1,N-1}))-3 : start_scenario{k,1}(length(start_scenario{k,1}),1);
                else
                    time{N} = time{1,N-1}(1,length(time{1,N-1}))+1 : (start_scenario{k,1}(length(start_scenario{k,1}),1));
                end
                ...
                filename = strcat(path,'PSD_BPF_0_50','.xlsx');
                ...
                for range_of_freq = 1:3
                    ...
                    if strcmpi(table2array(alarm_name{k,m}(1,1)),'F102_low') || strcmpi(table2array(alarm_name{k,m}(1,1)),'F102_high')
                        ...
                        subplot(3,2,[5,6])
                        x = [(start_scenario{k,1}(1,1)+ start_alarm(1,1))/2,(start_alarm(1,1) + start_scenario{k,1}(4,1))/2,mean_time_plotting] ;
                        y = PSD;
