...
%             fprintf(fid,'%.2f\n',filteredData_data);
%             fclose(fid);
%             save('eeg_data_filtered.txt','filteredData_data','-ascii')
        end
        ...
        for k=1:length(Folder)
            ...
            for m =1:6
                ...
                start_scenario{k,m} = eval(strcat(participant,sch,'_expt_',num2str(k),'_mouse_click',num2str(m)));
                start_alarm{k,m} = eval(strcat(participant,sch,'_expt_',num2str(k),'_scenario_',num2str(m)));
                ...
                figure(L)
                subplot(2,1,1)
                pop_spectopo(EEG, 1, [(diff_rest + start_scenario{k,m}(1,1))*512  (diff_rest+ start_alarm{k,m}(1,1))*512], 'EEG' , 'freqrange',[2 30],'electrodes','on'); % Start to Start of task
                ...
                subplot(2,1,2)
                if start_scenario{k,m} (4,1) - start_scenario{k,m}(1,1) == 1 || start_scenario{k,m} (4,1) - start_scenario{k,m}(1,1) == 0
                    time2 = start_scenario{k,m} (4,1) + 2;
                else
                    time2 = start_scenario{k,m} (4,1);
                end
                pop_spectopo(EEG, 1, [(diff_rest + start_scenario{k,m}(1,1))*512 (diff_rest + time2)*512], 'EEG' , 'freqrange',[2 30],'electrodes','on'); % Task to 1st slider action
                ...
                diff = abs(start_scenario{k,m}(4,1) - start_scenario{k,m}(length(start_scenario{k,m}),1));
                N = ceil(diff/10);
                time1 = diff_rest + start_scenario{k,m}(4,1);
                time2 = diff_rest + 10+start_scenario{k,m}(4,1);
                for i = 1:N-1
                ...
                time_action{N} = time_action{1,N-1}(1,length(time_action{1,N-1}))+1 : (diff_rest + start_scenario{k,m}(length(start_scenario{k,m}),1));
                if length(time_action{1,N})<3
                    
                    time_action{N} = time_action{1,N-1}(1,length(time_action{1,N-1}))-3 : diff_rest+start_scenario{k,m}(length(start_scenario{k,m}),1);
                else
                    time_action{N} = time_action{1,N-1}(1,length(time_action{1,N-1}))+1 : (diff_rest + start_scenario{k,m}(length(start_scenario{k,m}),1));
                end
                ...
                diff = abs(start_scenario{k,m}(4,1) - start_scenario{k,m}(length(start_scenario{k,m}),1));
                N = ceil(diff/10);
                time1 = start_scenario{k,m}(4,1);
                time2 = 10+start_scenario{k,m}(4,1);
                for i = 1:N-1
                    time{i} = ((i-1)*10)+time1+1:((i-1)*10)+time2;
                    time_mean_plotting{i} = floor(mean(cell2mat(time(i))));
                end
                time{N} = time{1,N-1}(1,length(time{1,N-1}))+1 : (start_scenario{k,m}(length(start_scenario{k,m}),1));
                if length(time{1,N})<3
                    
                    time{N} = time{1,N-1}(1,length(time{1,N-1}))-3 : start_scenario{k,m}(length(start_scenario{k,m}),1);
                else
                    time{N} = time{1,N-1}(1,length(time{1,N-1}))+1 : (start_scenario{k,m}(length(start_scenario{k,m}),1));
                end
                ...
                filename = strcat(path,'Experiment_new_rest','.xlsx');
                ...
                for range_of_freq = 1:3
                    ...
                    if strcmpi(table2array(alarm_name{k,m}(1,1)),'F102_low') || strcmpi(table2array(alarm_name{k,m}(1,1)),'F102_high')
                        ...
                        subplot(3,2,[5,6])
                        x = [(start_scenario{k,m}(1,1)+ start_scenario{k,m}(1,1))/2,(start_scenario{k,m}(1,1) + start_scenario{k,m}(4,1))/2,mean_time] ;
                        y = PSD;
