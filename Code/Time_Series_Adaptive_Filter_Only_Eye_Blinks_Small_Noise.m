...
%         end
        ...
        fnm =[Parent_Folder strcat(participant,'.',num2str(k)) '\' 'eeg_data_Adaptive_filter_WOI_Eyeblinkandsmallnoise_4_Order.txt'];
        fid = fopen(fnm,'wt');
        fprintf(fid,'%.2f\n',eeg_Adaptive_filtered_data);
        fclose(fid);
        save('eeg_data_Adaptive_filter_WOI_Eyeblinkandsmallnoise_4_Order.txt','data_to_be_filtered','-ascii')
    end
end