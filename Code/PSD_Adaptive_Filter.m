...
for destination = 1
    ...
    for i = 1
        ...
        for k=1:length(Folder)
            ...
            %             fnm =[Parent_Folder strcat(participant,'.',num2str(k)) '\' 'eeg_data_filtered_new.txt'];
            %             fid = fopen(fnm,'wt');
            %             fprintf(fid,'%.2f\n',filteredData_data);
            %             fclose(fid);
            %             save('eeg_data_filtered_new.txt','filteredData_data','-ascii')
            
            
            %     save(strcat('Participant',num2str(destination),'_',num2str(lowEnd),'_',num2str(highEnd),'Hz.mat'));
        end
        ...
        for k=1:length(Folder)
            ...
            ip_signal_raw{destination,1}{k} = load(FileList);
            
            % Raw data between -100 and 100 uV
            for i = 1:length(ip_signal_raw{destination,1}{k})
                if ip_signal_raw{destination,1}{k}(i,1) < -100 || ip_signal_raw{destination,1}{k}(i,1) > 100
                    ip_signal_100_100{destination,1}{k}(i,1)  = NaN; % ip_signal_100_100 contains NaN , ip_signal_raw IS RAW DATA, ip_signal_wo_nan is without NaN or within +-100 uV
                else
                    ip_signal_100_100{destination,1}{k}(i,1)  = ip_signal_raw{destination,1}{k}(i,1);
                end
            end
            mask = cellfun(@(C) isnumeric(C) && isscalar(C) && isnan(C), num2cell(ip_signal_100_100{destination,1}{k}));
            ip_signal_nan{destination,1}{k} = num2cell(ip_signal_100_100{destination,1}{k});
            ip_signal_nan{destination,1}{k}(mask) = {[]};
            ip_signal_wo_nan{destination,1}{k} = cell2mat(ip_signal_nan{destination,1}{k});
            %             Saving the Good eeg data to desired folder
            fnm =[Parent_Folder strcat(participant,'.',num2str(k)) '\' 'eeg_AF.txt'];
            fid = fopen(fnm,'wt');
            fprintf(fid,'%.2f\n',ip_signal_wo_nan{destination,1}{k});
            fclose(fid);
            save('eeg_AF.txt','eeg_AF','-ascii')
            for m =1:6
                %Start to 1st alarm
                %EEG.etc.eeglabvers = '2019.0'; % this tracks which version of EEGLAB is being used, you may ignore it
                [ALLEEG EEG CURRENTSET ALLCOM] = eeglab;
                EEG = pop_importdata('dataformat','ascii','nbchan',1,'data',strcat(Parent_Folder,participant,'.',num2str(k),'\eeg_AF.txt'),'setname','applicant','srate',512,'pnts',1,'xmin',0);
                ...
                subplot(3,1,1)
                if Delta_EEG_eye_tracker{destination,1}(k,1) <= 120
                    diff = abs(Delta_EEG_eye_tracker{destination,1}(k,1));
                else
                    diff = abs(120);
                end
                N = ceil(diff/10);
                time2 = 10;
                clear x_1 y_1 time_rest S_theta_rest S_alpha_rest S_beta_rest mean_time_rest time_mean_rest time
                for i = 1:N-1
                    time{i} = ((i-1)*10)+1:((i-1)*10)+time2;
                    time1 = time{1,i}(1,1)*512- sum(isnan(ip_signal_100_100{destination,1}{k}(1:time{1,i}(1,1)*512,1)));
                    time2 = time1 + time{1,i}(1,length(time{1,i}))*512-time{1,i}(1,1)*512 - sum(isnan(ip_signal_100_100{destination,1}{k}(time{1,i}(1,1)*512:time{1,i}(1,length(time{1,i}))*512,1)));
                    ...
                    time_mean_rest{i} = floor(mean(cell2mat(time(i))));
                end
                time{N} = time{1,N-1}(1,length(time{1,N-1}))+1 : diff;
                if length(time{1,N})<3
                    
                    time{N} = time{1,N-1}(1,length(time{1,N-1}))-3 : diff;
                else
                    time{N} = time{1,N-1}(1,length(time{1,N-1}))+1 : diff;
                end
                time1 = time{1,N}(1,1)*512- sum(isnan(ip_signal_100_100{destination,1}{k}(1:time{1,N}(1,1)*512,1)));
                time2 = time1 + time{1,N}(1,length(time{1,N}))*512 - time{1,N}(1,1)*512 - sum(isnan(ip_signal_100_100{destination,1}{k}(time{1,N}(1,1)*512:time{1,N}(1,length(time{1,N}))*512,1)));
                pop_spectopo(EEG, 1, [time1 time2], 'EEG' , 'freqrange',[2 30],'electrodes','on'); % First slider action to completion of scenario
                h4 = findobj(gca,'Type','line');
                x_1{N}=get(h4,'Xdata');
                y_1{N}=get(h4,'Ydata');
                time_mean_rest{N} = floor(mean(time{N}));
                close all
                ...
                figure(L)
                subplot(3,1,2)
                time1 = start_scenario(1,1)*512- sum(isnan(ip_signal_100_100{destination,1}{k}(1:start_scenario(1,1)*512,1)));
                time2 = time1 + start_alarm(1,1)*512 - start_scenario(1,1)*512 - sum(isnan(ip_signal_100_100{destination,1}{k}(start_scenario(1,1)*512:start_alarm(1,1)*512,1)));
                ...
                time1 = start_alarm(1,1)*512- sum(isnan(ip_signal_100_100{destination,1}{k}(1:start_alarm(1,1)*512,1)));
                time2 = time1 + timet*512 - start_alarm(1,1)*512 - sum(isnan(ip_signal_100_100{destination,1}{k}(start_alarm(1,1)*512:timet*512,1)));
                ...
                filename = strcat(path,'Experiment_new_rest_for_AF','.xlsx');
