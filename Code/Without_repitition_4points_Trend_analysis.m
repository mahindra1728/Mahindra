...
for destination = 1
    ...
    for i = 1
        ...
        for k=1:length(Folder_normal)
            ...
            for m=1:6
                ...
                [~,txtData1{k,m}]=xlsread(read{1},read{2});
                ...
                % Theta Index
                if mean_alarm2slider_theta{k,m}>mean_SC2finish_theta{k,m}
                    index_slider_finish_theta{k,m} =1;
                else
                    index_slider_finish_theta{k,m} = 0;
                end
                
                if mean_start2alarm_theta{k,m}>mean_SC2finish_theta{k,m}
                    index_start_finish_theta{k,m} = 1;
                else
                    index_start_finish_theta{k,m} = 0;
                end
                
                % Beta index
                if mean_alarm2slider_beta{k,m}>mean_SC2finish_beta{k,m}
                    index_slider_finish_beta{k,m} =1;
                else
                    index_slider_finish_beta{k,m} = 0;
                end
                
                if mean_start2alarm_beta{k,m}>mean_SC2finish_beta{k,m}
                    index_start_finish_beta{k,m} = 1;
                else
                    index_start_finish_beta{k,m} = 0;
                end
                
                % Alpha Index
                if mean_alarm2slider_alpha{k,m}<mean_SC2finish_alpha{k,m}
                    index_slider_finish_alpha{k,m} =1;
                else
                    index_slider_finish_alpha{k,m} = 0;
                end
                
                if mean_start2alarm_alpha{k,m}<mean_SC2finish_alpha{k,m}
                    index_start_finish_alpha{k,m} = 1;
                else
                    index_start_finish_alpha{k,m} = 0;
                end
                
                
                %% Finding failed indexes in the mouseclick file
    ...
    indexfail{destination,1} = index_fail;
    percent_slider2finish_alpha{destination}  = sum(sum(cell2mat(index_slider_finish_alpha)))/(0.01*6*length(Folder_normal));
    percent_slider2finish_beta{destination}   = sum(sum(cell2mat(index_slider_finish_beta)))/(0.01*6*length(Folder_normal));
    percent_slider2finish_theta{destination}  = sum(sum(cell2mat(index_slider_finish_theta)))/(0.01*6*length(Folder_normal));
    
    percent_start2finish_alpha{destination}   = sum(sum(cell2mat(index_start_finish_alpha)))/(0.01*6*length(Folder_normal));
    percent_start2finish_beta{destination}    = sum(sum(cell2mat(index_start_finish_beta)))/(0.01*6*length(Folder_normal));
    percent_start2finish_theta{destination}   = sum(sum(cell2mat(index_start_finish_theta)))/(0.01*6*length(Folder_normal));
end
...
%         length_index =  1:length(mean_start2alarm_alpha(:,1));
%
%         %% Alpha variation in 4 parts by repitition column 1 is normal column2 is random
%         y_alpha_1_1 = mean_start2alarm_alpha(:,1);% Part1 Scenario1 alpha variation with repition
%         y_alpha_1_2 = mean_start2alarm_alpha(:,2);% Part1 Scenario2 alpha variation with repition
%         y_alpha_1_3 = mean_start2alarm_alpha(:,3);% Part1 Scenario3 alpha variation with repition
%         y_alpha_1_4 = mean_start2alarm_alpha(:,4);% Part1 Scenario4 alpha variation with repition
%         y_alpha_1_5 = mean_start2alarm_alpha(:,5);% Part1 Scenario5 alpha variation with repition
%         y_alpha_1_6 = mean_start2alarm_alpha(:,6);% Part1 Scenario6 alpha variation with repition
%
%         y_alpha_2_1 = mean_alarm2slider_alpha(:,1);% Part2 Scenario1 alpha variation with repition
%         y_alpha_2_2 = mean_alarm2slider_alpha(:,2);% Part2 Scenario2 alpha variation with repition
%         y_alpha_2_3 = mean_alarm2slider_alpha(:,3);% Part2 Scenario3 alpha variation with repition
%         y_alpha_2_4 = mean_alarm2slider_alpha(:,4);% Part2 Scenario4 alpha variation with repition
%         y_alpha_2_5 = mean_alarm2slider_alpha(:,5);% Part2 Scenario5 alpha variation with repition
%         y_alpha_2_6 = mean_alarm2slider_alpha(:,6);% Part2 Scenario6 alpha variation with repition
%
%         y_alpha_3_1 = mean_slider2SC_alpha(:,1);% Part3 Scenario1 alpha variation with repition
%         y_alpha_3_2 = mean_slider2SC_alpha(:,2);% Part3 Scenario2 alpha variation with repition
%         y_alpha_3_3 = mean_slider2SC_alpha(:,3);% Part3 Scenario3 alpha variation with repition
%         y_alpha_3_4 = mean_slider2SC_alpha(:,4);% Part3 Scenario4 alpha variation with repition
%         y_alpha_3_5 = mean_slider2SC_alpha(:,5);% Part3 Scenario5 alpha variation with repition
%         y_alpha_3_6 = mean_slider2SC_alpha(:,6);% Part3 Scenario6 alpha variation with repition
%
%         y_alpha_4_1 = mean_SC2finish_alpha(:,1);% Part4 Scenario1 alpha variation with repition
%         y_alpha_4_2 = mean_SC2finish_alpha(:,2);% Part4 Scenario2 alpha variation with repition
%         y_alpha_4_3 = mean_SC2finish_alpha(:,3);% Part4 Scenario3 alpha variation with repition
%         y_alpha_4_4 = mean_SC2finish_alpha(:,4);% Part4 Scenario4 alpha variation with repition
%         y_alpha_4_5 = mean_SC2finish_alpha(:,5);% Part4 Scenario5 alpha variation with repition
%         y_alpha_4_6 = mean_SC2finish_alpha(:,6);% Part4 Scenario6 alpha variation with repition
%
%         %% Beta variation in 4 parts by repitition column 1 is normal column2 is random
%         y_beta_1_1 = mean_start2alarm_beta(:,1);% Part1 Scenario1 beta variation with repition
%         y_beta_1_2 = mean_start2alarm_beta(:,2);% Part1 Scenario2 beta variation with repition
%         y_beta_1_3 = mean_start2alarm_beta(:,3);% Part1 Scenario3 beta variation with repition
%         y_beta_1_4 = mean_start2alarm_beta(:,4);% Part1 Scenario4 beta variation with repition
%         y_beta_1_5 = mean_start2alarm_beta(:,5);% Part1 Scenario5 beta variation with repition
%         y_beta_1_6 = mean_start2alarm_beta(:,6);% Part1 Scenario6 beta variation with repition
%
%         y_beta_2_1 = mean_alarm2slider_beta(:,1);% Part2 Scenario1 beta variation with repition
%         y_beta_2_2 = mean_alarm2slider_beta(:,2);% Part2 Scenario2 beta variation with repition
%         y_beta_2_3 = mean_alarm2slider_beta(:,3);% Part2 Scenario3 beta variation with repition
%         y_beta_2_4 = mean_alarm2slider_beta(:,4);% Part2 Scenario4 beta variation with repition
%         y_beta_2_5 = mean_alarm2slider_beta(:,5);% Part2 Scenario5 beta variation with repition
%         y_beta_2_6 = mean_alarm2slider_beta(:,6);% Part2 Scenario6 beta variation with repition
%
%         y_beta_3_1 = mean_slider2SC_beta(:,1);% Part3 Scenario1 beta variation with repition
%         y_beta_3_2 = mean_slider2SC_beta(:,2);% Part3 Scenario2 beta variation with repition
%         y_beta_3_3 = mean_slider2SC_beta(:,3);% Part3 Scenario3 beta variation with repition
%         y_beta_3_4 = mean_slider2SC_beta(:,4);% Part3 Scenario4 beta variation with repition
%         y_beta_3_5 = mean_slider2SC_beta(:,5);% Part3 Scenario5 beta variation with repition
%         y_beta_3_6 = mean_slider2SC_beta(:,6);% Part3 Scenario6 beta variation with repition
%
%         y_beta_4_1 = mean_SC2finish_beta(:,1);% Part4 Scenario1 beta variation with repition
%         y_beta_4_2 = mean_SC2finish_beta(:,2);% Part4 Scenario2 beta variation with repition
%         y_beta_4_3 = mean_SC2finish_beta(:,3);% Part4 Scenario3 beta variation with repition
%         y_beta_4_4 = mean_SC2finish_beta(:,4);% Part4 Scenario4 beta variation with repition
%         y_beta_4_5 = mean_SC2finish_beta(:,5);% Part4 Scenario5 beta variation with repition
%         y_beta_4_6 = mean_SC2finish_beta(:,6);% Part4 Scenario6 beta variation with repition
%
%         %% Theta variation in 4 parts by repitition column 1 is normal column2 is random
%         y_theta_1_1 = mean_start2alarm_theta(:,1);% Part1 Scenario1 theta variation with repition
%         y_theta_1_2 = mean_start2alarm_theta(:,2);% Part1 Scenario2 theta variation with repition
%         y_theta_1_3 = mean_start2alarm_theta(:,3);% Part1 Scenario3 theta variation with repition
%         y_theta_1_4 = mean_start2alarm_theta(:,4);% Part1 Scenario4 theta variation with repition
%         y_theta_1_5 = mean_start2alarm_theta(:,5);% Part1 Scenario5 theta variation with repition
%         y_theta_1_6 = mean_start2alarm_theta(:,6);% Part1 Scenario6 theta variation with repition
%
%         y_theta_2_1 = mean_alarm2slider_theta(:,1);% Part2 Scenario1 theta variation with repition
%         y_theta_2_2 = mean_alarm2slider_theta(:,2);% Part2 Scenario2 theta variation with repition
%         y_theta_2_3 = mean_alarm2slider_theta(:,3);% Part2 Scenario3 theta variation with repition
%         y_theta_2_4 = mean_alarm2slider_theta(:,4);% Part2 Scenario4 theta variation with repition
%         y_theta_2_5 = mean_alarm2slider_theta(:,5);% Part2 Scenario5 theta variation with repition
%         y_theta_2_6 = mean_alarm2slider_theta(:,6);% Part2 Scenario6 theta variation with repition
%
%         y_theta_3_1 = mean_slider2SC_theta(:,1);% Part3 Scenario1 theta variation with repition
%         y_theta_3_2 = mean_slider2SC_theta(:,2);% Part3 Scenario2 theta variation with repition
%         y_theta_3_3 = mean_slider2SC_theta(:,3);% Part3 Scenario3 theta variation with repition
%         y_theta_3_4 = mean_slider2SC_theta(:,4);% Part3 Scenario4 theta variation with repition
%         y_theta_3_5 = mean_slider2SC_theta(:,5);% Part3 Scenario5 theta variation with repition
%         y_theta_3_6 = mean_slider2SC_theta(:,6);% Part3 Scenario6 theta variation with repition
%
%         y_theta_4_1 = mean_SC2finish_theta(:,1);% Part4 Scenario1 theta variation with repition
%         y_theta_4_2 = mean_SC2finish_theta(:,2);% Part4 Scenario2 theta variation with repition
%         y_theta_4_3 = mean_SC2finish_theta(:,3);% Part4 Scenario3 theta variation with repition
%         y_theta_4_4 = mean_SC2finish_theta(:,4);% Part4 Scenario4 theta variation with repition
%         y_theta_4_5 = mean_SC2finish_theta(:,5);% Part4 Scenario5 theta variation with repition
%         y_theta_4_6 = mean_SC2finish_theta(:,6);% Part4 Scenario6 theta variation with repition
%
