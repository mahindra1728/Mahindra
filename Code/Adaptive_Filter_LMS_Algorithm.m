...
for destination = 1%:10
    ...
    warning('off')
    format short g
    Main_Folder1 = 'F:\Adaptive filter\PSD_Filters\From Rest Period\4_points_to_10seconds_expt_file\4_points_new\4_point\EEGDATA\';
    Main_Folder2 = 'F:\Adaptive filter\PSD_Filters\From Rest Period\4_points_to_10seconds_expt_file\4_points_new\4_point\EEGDATA\';
    ...
    %% Reading the Desired PSD Data
        ...
        for k=1%:length(Folder2)
            ...
            mu=0;
            length = zeros(100000,1);
            iteration = 100000;
            for i = 1:100000
                length(i) = 0 + 1/100000;
                mu = mu+length(i);
                lms = dsp.LMSFilter(13,'StepSize',mu);
                [y2,e,w] = lms(Actual_theta',desired_theta');
                y2 = y2';
                sum_error(i) = sum((y2'-Actual_theta').^2);
            end
            [value,index] = min(sum_error);
            index_mu = sort(index);
            value_mu = 0+index_mu/100000;
                mu = value_mu;
                lms = dsp.LMSFilter(13,'StepSize',mu);
                [y2,e,w] = lms(Actual_theta',desired_theta');
            Actual_theta  = mean(PSD_Actual{destination,1}{k}(5:8,:));  % Actual Data
