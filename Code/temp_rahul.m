...
%                             title(strcat('PSD ',Properties(plotstat),' COMPARISON IN REST AND ACTION PHASE FOR ',name_participant{destination}))
                        ...
                        a = axes;
                        t1 = title({strcat(name_participant,'EXPERIMENT',num2str(k),upper(waves(wave)),'-',Properties(plotstat));''});
                        a.Visible = 'off'; % set(a,'Visible','off');
                        t1.Visible = 'on'; % set(t1,'Visible','on');
                        set(figure(plotstat), 'units','normalized','outerposition',[0 0 1 1])
                        saveas(figure(plotstat),fullfile(strcat(Parent_Folder),[strcat(name_participant,' EXPERIMENT- ',num2str(k),' PSD- ',upper(waves(wave)),'-',Properties(plotstat),'_TASK_ALL', '.png')]));
