...
for destination = 1%:10
    ...
    warning('off')
    format short g
    Main_Folder = 'F:\From Rest Period\4_points_to_10seconds_expt_file\4_points_new\4_point\EEGDATA\';
  ...
  %% EEGLAB
        ...
        for k=1:length(Folder)
            ...
            for m =1:6
                ...
                EEG = pop_importdata('dataformat','ascii','nbchan',1,'data',strcat(Parent_Folder,participant,'.',num2str(k),'\eeg_data_filtered.txt'),'setname','applicant','srate',512,'pnts',1,'xmin',0);
                ...
                subplot(3,1,1)
                if Delta_EEG_eye_tracker{destination,1}(k,1) <= 120
                    diff = abs((start_scenario(1,1)+Delta_EEG_eye_tracker{destination,1}(k,1)));
                else
                    diff = abs((start_scenario(1,1)+ 120));
                end
                N = ceil(diff/10);
                time2 = 10;
                clear x_1 y_1 time time_rest S_theta_rest S_alpha_rest S_beta_rest mean_time_rest time_mean_rest
                for i = 1:N-1
                ...
                if start_scenario(4,1) - start_alarm(1,1) == 1 || start_scenario(4,1) - start_alarm(1,1) == 0
                    time2 = start_scenario(4,1) + 2
                else
                    time2 = start_scenario(4,1)
                end
                pop_spectopo(EEG, 1, [(start_alarm(1,1))*512 (time2)*512], 'EEG' , 'freqrange',[2 30],'electrodes','on'); % Task to 1st slider action
                ...
                data_PSD = [y_3_10seconds_rest y_2' y_3' y_3_10seconds];
                sheet = m;
                xlRange = 'A';
