...
%         
        ...
        for k=1:length(Folder1)
            ...
            disp(path)
            counts_noise_new = counts_noise{destination,1}{k};
            filt_data = load(strcat(path,'\eeg_data_Adaptive_filter_WI.txt'));
            ...
            if (destination == 1 && k==5) || (destination == 2 && k==6) || (destination == 3 && k==8) || (destination == 6 && k==2) || (destination == 8 && k==1) || (destination == 10 && k==5 ||destination == 10 && k==6)
                ...
                for len = a:length(counts_noise_new(:,1))
                    if counts_noise_new(len,3) > 15
                        ...
                        if len == length(counts_noise_new(:,1))
                            correlation = corrcoef(filt_data(counts_noise_new(len,1):counts_noise_new(end,2)),filt_data(counts_noise_new(len,1)-diff:counts_noise_new(len,1)));
                        else
                            correlation = corrcoef(filt_data(counts_noise_new(len,1):counts_noise_new(len,2)),filt_data(counts_noise_new(len,1)-diff:counts_noise_new(len,1)));
                        end
                        counts_noise_new(len,5) = correlation(1,2);
                    end
                end
            else
                ...
                for len = 3:length(counts_noise_new(:,1))
                    if counts_noise_new(len,3) > 15
                        diff = abs(counts_noise_new(len,1) - counts_noise_new(len,2));
                        if len == length(counts_noise_new(:,1))
                            correlation = corrcoef(filt_data(counts_noise_new(len,1):end),filt_data(counts_noise_new(len,1)-diff+1:counts_noise_new(len,1)));
                        else
                            correlation = corrcoef(filt_data(counts_noise_new(len,1):counts_noise_new(len,2)),filt_data(counts_noise_new(len,1)-diff:counts_noise_new(len,1)));
                        end
                        counts_noise_new(len,5) = correlation(1,2);
