...
correlation = zeros(10,1);
correlation_overall = zeros(10,1);
correlation_mean = zeros(10,11);
...
for destination = 1:10
    ...
    warning('off')
    format short g
    Main_Folder = 'D:\Mahindra\PSD_0.5_100\From Rest Period\4_points_to_10seconds_expt_file\4_points_new\4_point\EEGDATA\';
    ...
    for k=1:length(Folder1)
        ...
        filt_data = load(strcat(path,'\eeg_data_BP_0_50.txt'));
        raw_data  = load(strcat(path,'\eeg_data.txt'));
        if (u == 1 && k==5) || (u == 2 && k==6)|| (u == 5 && k==8) || (u == 3 && k==8) || (u == 6 && (k==2 || k==8)) ||(u == 8 && (k==3 || k==5 || k==1)) || (u == 10 && (k==5 || k==6)) || (u == 9 && (k==2 || k==8))
            if u == 8 || u == 10
                a = 4;
            else
                a = 3;
            ...
            for len = 2:length(counts_noise_new(:,1))
                if counts_noise_new(len,3) >= 15
                    diff = abs(counts_noise_new(len,1) - counts_noise_new(len,2));
                    if len == length(counts_noise_new(:,1))
        ...
        correlation_mean(u,k) = mean(correlation_overall{u,1}{k});
    end
    correlation_mean(u,10) = mean(correlation_mean(u,1:k));
    correlation_mean(u,11) = std(correlation_mean(u,1:k));
end
correlation_mean(11,10:11) =  mean(correlation_mean(:,10:11));
