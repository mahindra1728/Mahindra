...
for destination = 1:10
    ...
    for k=1:length(Folder)
        ...
        data_to_be_filtered{u,1}{k} = ip_signal_raw{destination,1}{k};
        ...
        for j = 1:length(counts_noise_new(:,1))
            ...
            if abs(counts_noise_new(row,1) - counts_noise_new(row,2)-1) >= 10
                ...
                data_to_be_filtered{u,1}{k}(counts_noise_new(row,1):counts_noise_new(row,2)) = movmean(data_to_be_filtered(counts_noise_new(row,1)-2:counts_noise_new(row,1)-1),2);
            end
        end
        ...
        fnm =[Parent_Folder strcat(participant,'.',num2str(k)) '\' 'eeg_data_Adaptive_filter_WI_2degree.txt'];
        fid = fopen(fnm,'wt');
        fprintf(fid,'%.2f\n',eeg_Adaptive_filtered_data);
        fclose(fid);
        save('eeg_data_Adaptive_filter_WI_2degree.txt','data_to_be_filtered','-ascii')
    end
end