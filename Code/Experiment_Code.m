...
%% Importing the eye tracker data
        ...
        for k=1:length(Folder)
            ...
            for m=1
                ...
                name = eval(strcat('name','_',num2str(k),'_',num2str(1)));
...
%% EEG Timestamp
        ...
        for k=1:length(Folder)
            ...
            for m=1
                ...
                ti1e_eeg_record{k,1} = EEG_timestamp{1,1}.Hour*3600 + EEG_timestamp{1,1}.Minute*60 + EEG_timestamp{1,1}.Second; % Ti1e in seconds EEG started recording the data
                u = table2array(ti1e_eeg_record(k,1));
...
%% PLOTTING THE PROCESS DATA EXPERIMENT AND SCENARIO WISE
                ...
                elseif strcmpi(table2array(alarm_name{k,m}(1,1)), 'F101_high') || strcmpi(table2array(alarm_name{k,m}(1,1)),'F101_low')
                    figure(m)
                    subplot(3,1,1)
                    plot(time,process_raw{k,m}{1,1}(:,3))
                    ...
                    subplot(3,1,2)
                    plot(time,process_raw{k,m}{1,1}(:,7))
                    ...
                    subplot(3,1,3)
                    plot(time,process_raw{k,m}{1,1}(:,12))
                    ...
                    figure(m)
                    subplot(3,1,1)
                    plot(time,process_raw{k,m}{1,1}(:,9))
                    ...
                    subplot(3,1,2)
                    plot(time,process_raw{k,m}{1,1}(:,8))
                    ...
                    subplot(3,1,3)
                    plot(time,process_raw{k,m}{1,1}(:,10))
                ...
                print(figure(m),fullfile(path,['Processdata_expt_',num2str(k),'_Task_',num2str(m),'_part_all', '.png']),'-dpng','-r700');
